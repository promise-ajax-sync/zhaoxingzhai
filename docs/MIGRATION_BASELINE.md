...
| 历法基础 | 迁移中 | IANA 历史时区和 DST 已使用 timezone 数据库；术式历法快照已分离农历年界与节气干支年界；真太阳时与中国夏令时仍需补齐上游测试 |
| 三山国王灵签 | 基础迁移完成 | 92 签完整数据、随机/手动查签、seed/replay、算法版本、UI、历史、案例快照和 3 组 mingyu golden 已接入 |
| 每日一卦 | 业务闭环完成 | 三钱六次起卦、逐枚铜钱录入与动画、本卦/变卦/互卦、动爻辞、0–6 动爻取用规则、独立版本化分项解读、乾坤用辞、v3 golden、确定性日卦、UI、历史、案例快照及 v1/v2/v3 历史恢复已接入 |
| 梅花易数 | 基础闭环 | 时间/数字/随机起卦、主互变、动爻、体用关系、固定向量、历法边界、页面、案例快照和历史恢复已接入 |
| 其他术式 | 未开始 | 按六爻，再到复杂排盘的顺序推进 |

算法实施细目见 [`ALGORITHM_STATUS.md`](ALGORITHM_STATUS.md)。导航中出现入口不等于对应算法已经迁移。

梅花易数首批范围及历法阻塞说明见 [`MEIHUA_MIGRATION_BASELINE.md`](MEIHUA_MIGRATION_BASELINE.md)。
